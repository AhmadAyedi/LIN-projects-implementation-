...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('wiper_slave_can.log'),
...
class WiperSlave:
    ...
    def run(self):
        try:
            logging.info("Starting CAN wiper slave")
            while True:
                try:
                    # Wait for a message with timeout to allow for KeyboardInterrupt
                    msg = self.bus.recv(timeout=0.1)
                    
                    if msg and msg.arbitration_id == 0x123 and len(msg.data) == 3:
                        data = msg.data
            ...
            self.bus.shutdown()
            GPIO.cleanup()
            logging.info("CAN slave shutdown complete")
